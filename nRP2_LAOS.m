function  [HTMoC, HTMnC] = nRP2_LAOS(model, De, gamma0, Z, ncyc, ncyc_sol, nfm)
%This script performs a stability analysis of a LAOS flow using the nRP2 model.
%It perturbs the system with Fourier modes and examines the response to
%these perturbations.
...
%nRP-2
x0 = [0, 1, 1, 0, 0, 0]; %Initial condition depends on what ODE system (model) are you solving
base_LC = get_base_LC_solution_nRP2(model, x0, dt, gdot, ddtgdot, De, gamma0, nstep, ncyc, ncyc_sol, Z);
base_sxy0 = base_LC(1, :);
% base_sxy = base_LC(4, :) / Z;
base_sxy = base_LC(4, :);
% base_sxy = base_LC(1, :);
x0 = base_LC(:, end)';
...
HTMoC = cell(length(omega),1);
HTMnC = cell(length(omega),1);
...
[yP, yo] = perturb_response_nRP2 (model, nfm, x0, dt, gdot, fourier_modes, ddtgdot, ...
    ddt_fourier_modes, De, gamma0, nstep, ncyc,ncyc_sol, Z, base_sxy0, base_sxy,s,f);
...
HTMo = zeros(2*nfm+1, 2*nfm+1);
HTMn = zeros(2*nfm+1, 2*nfm+1);
                        ...
                        for n = -nfm:nfm     
                            ...
                            for m = 1:2*nfm+1
                                ...
                                id = -1i*(f*(n)).*tspan;
                                 ...
                                 HTMo (index, m)  =  sum(yo (m,nstep*(ncyc_sol-1)+1:end).* exp(id)) * dt/pe;
                                 HTMn (index, m)  =  sum(yP (m,nstep*(ncyc_sol-1)+1:end).* exp(id)) * dt/pe;
...
%% Adaptation to reduce computational cost.
...
%% Previous version of the code
                        %HTM = HTMo + 1/Z*(HTMn) (unperturbed sys + perturbation)
                        % HTM = HTMo+(1/Z)*HTMn;
                 ...
                 %% Perturbation Theory (Non-degenerated case)
                 % [evals, ~] = Perturbation_theory (HTMo, HTM,Z);
                 % 
                 %    HTM_eval (:, iter) = evals; 
...
% HTM_block = (blkdiag(HTM_collection{:}));

%% HTM decomposition (LTI + off-diagonal terms)

% HTM_decomposition (HTM_collection, omega, nfm, De, Z, gamma0)

%% Nyquist Plot
% num_eval = 2*nfm+1; % Number of evals used to generate the Nyquist plot
% Nyquist_plot(HTM_eval, num_eval, nfm, De, Z, gamma0)
